...
function loadingScreen(elementId, typingSpeed){
    ...
    currentElement.append(cursor);
    var loadCursor = document.getElementById("loading-cursor");
    currentElement = document.getElementById("loading-text");
    var text = currentElement.innerText.trim().split('');
    ...
    setTimeout(function(){
        ...
        for(var i = 0; i < amntOfChars; i++){
            (function(i,char){
                setTimeout(function() {
                    newString += char;
                    currentElement.innerText = newString;
                },i*typingSpeed);
            })(i+1,text[i]);
        }
    },1500);
...
function makeResizableDiv(div) {
    ...
    currentResizer.addEventListener('touchstart', function(e) {
        ...
        originalX = element.getBoundingClientRect().left;
        originalY = element.getBoundingClientRect().top;
        originalMouseX = e.pageX || e.touches[0].clientX;
        originalMouseY = e.pageY || e.touches[0].clientY;
        element.classList.add("resizing");
